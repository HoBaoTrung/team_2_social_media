...
class PostManager {
    ...
    console.log(url);
        try {
            ...
            if (type === 'post') {
                ...
                isLiked = data.comment.likedByCurrentUser;
                likeCount = data.comment.likeCount;
            }

            // Cập nhật UI nút Like
            this.updateLikeButton(targetId, isLiked, type);
            this.updateLikeCount(targetId, likeCount, type);
    ...
    updateLikeCount(id, count, type) {
        let countSpan;
        if (type === 'post') {
            countSpan = document.querySelector(`#post-like-${id} span`);
        } else if (type === 'comment') {
            countSpan = document.querySelector(`#comment-like-${id} span`);
        }

        if (countSpan) countSpan.textContent = count; // ✅ update số like
    }
    ...
    subscribeToCommentLikes(commentId) {
        this.stompClient.subscribe(`/topic/comments/${commentId}/likes`, (message) => {
            const data = JSON.parse(message.body);
            this.updateLikeButton(data.commentId, data.likedByCurrentUser, 'comment');
...
// ===== Tự lấp đầy cho đến khi có thanh cuộn (đề phòng size quá nhỏ) =====
    ...
    appendCommentToUI(postId, c, type = 'append', parentId = null) {
        ...
        const actionButtons = (canEdit || canDelete) ? `
            <div class="dropdown comment-actions-dropdown">
                ...
                <ul class="dropdown-menu">
                    ${canEdit ? `<li><button class="dropdown-item" onclick="postManager.editCommentUI(${postId}, ${c.commentId})"><i class="fas fa-edit"></i> Sửa</button></li>` : ''}
                    ${canDelete ? `<li><button class="dropdown-item text-danger" onclick="postManager.deleteComment(${postId}, ${c.commentId})"><i class="fas fa-trash"></i> Xóa</button></li>` : ''}
                </ul>
            </div>
        ` : '';
                ...
                <div class="comment-body">
                    ...
                    <div class="comment-actions d-flex align-items-center">
                        <span class="like-btn ${c.likedByCurrentUser ? 'liked' : ''}" id="comment-like-${c.commentId}" 
                            ...
                            <i class="fas fa-heart ${c.likedByCurrentUser ? 'text-danger' : ''}"></i>
                            <span>${likeCount}</span>
                        </span>
                        <button class="btn btn-link btn-sm text-primary reply-btn" 
        ...
        // Subscribe realtime like cho comment mới
